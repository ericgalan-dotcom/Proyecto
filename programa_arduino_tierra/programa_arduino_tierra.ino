...
//Sistema de las alarmas: temperatura y comunicación
...
// Timeout de llas comunicaciones
unsigned long lastReceiveTime = 0;
unsigned long nexttimeout = 0;
unsigned long timeouttime = 10000;

//Función que sirve para descomprmir los mensajes que se envían des del satélite: forma parte del código de comunicaciones más ligeras
void descomprimirMensaje(String lineaHex) {
  ...
  checksumStr.trim()
  // Verificar que la longitud sea válida (12 o 16 caracteres = 6 o 8 bytes)
  ...
  // Código para reconstruir los mensajes 
  ...
  //Código solo para si hay media (tipo_msg == 1 y 8 bytes)
  ...
  // Construir mensaje descomprimido con el formato que lee python
...
void loop() {
  ...
  if (mySerial.available()) 
    ...
    if (esMensajeComprimido) {
      ...
      // Procesar mensajes para las alarmas
      if (data == "FALLO") { 
        digitalWrite(alarmaPin, HIGH);
        alarmaActiva = true;
