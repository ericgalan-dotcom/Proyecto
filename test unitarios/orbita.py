...
fig, ax = plt.subplots()
orbit_plot, = ax.plot([], [], 'bo-', label='órbita del satélite', markersize=2)  # Line for the orbit with smaller markers
last_point_plot = ax.scatter([], [], color='red', s=50, label='último punto')  # Scatter plot for the last point

# Draw the Earth's surface as a circle
earth_circle = plt.Circle((0, 0), R_EARTH, color='orange', fill=False, label='Superficie de la tierra')
...
ax.set_title('Órbita del satélite')
ax.grid(True)
ax.legend()
